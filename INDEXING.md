...
### Gotchas
* As far as we can tell, the dor-services `to_solr` code that generates the Solr documents leaks memory.  As such, the delayed_job workers leak memory, and eventually reindexing workers will die if they run for long enough.  As such, large reindexing batches may require some babysitting, and restarting of workers.  We attempt to monitor worker processes with `Eye`.
...
* At present, each worker is essentially an instance of the Argo application.  So they all start pretty heavy, even before any leaks.
