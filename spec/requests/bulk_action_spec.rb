...
RSpec.describe 'BulkActionsController' do
  ...
  describe 'GET index' do
    ...
    it 'shows a button to create a new one' do
      get "/bulk_actions?action=index&controller=catalog&f[#{SolrDocument::FIELD_CURRENT_VERSION}][]=3&f[#{SolrDocument::FIELD_APO_ID}][]=druid%3Azw306xn5593&f[#{SolrDocument::FIELD_OBJECT_TYPE}][]=item&page=3"
      # this tests that page, action and controller are stripped out of the parameters
      url = "/bulk_actions/new?f%5B#{SolrDocument::FIELD_CURRENT_VERSION}%5D%5B%5D=3&f%5B#{SolrDocument::FIELD_APO_ID}%5D%5B%5D=druid%3Azw306xn5593&f%5B#{SolrDocument::FIELD_OBJECT_TYPE}%5D%5B%5D=item"
      expect(rendered).to have_link 'New Bulk Action', href: url
    end

    it 'shows a button without bulk action form params' do
      get "/bulk_actions?action=index&controller=catalog&f[#{SolrDocument::FIELD_CURRENT_VERSION}][]=3&f[#{SolrDocument::FIELD_APO_ID}][]=druid%3Azw306xn5593&f[#{SolrDocument::FIELD_OBJECT_TYPE}][]=item&page=3&download_access=world&view_access=stanford"
