...
RSpec.describe StateService do
  before do
    ...
    allow(workflow_client).to receive(:workflow_status).with(druid: pid, process: 'accessioning-initiate', workflow: 'assemblyWF').and_return('completed')
  ...
  describe '#object_state' do
    ...
    context 'if there is not an open version' do
      before do
        allow(workflow_client).to receive(:active_lifecycle).with(druid: pid, milestone_name: 'submitted', version: 3).and_return(true)
        allow(workflow_client).to receive(:active_lifecycle).with(druid: pid, milestone_name: 'opened', version: 3).and_return(false)
      end
    ...
    context 'if the object is accessioned, not submitted and not opened' do
      before do
        allow(workflow_client).to receive(:active_lifecycle).with(druid: pid, milestone_name: 'submitted', version: 3).and_return(false)
        allow(workflow_client).to receive(:active_lifecycle).with(druid: pid, milestone_name: 'opened', version: 3).and_return(false)
        allow(workflow_client).to receive(:lifecycle).with(druid: pid, milestone_name: 'accessioned').and_return(true)
      end
  ...
  describe '#published?' do
    ...
    context 'if the published lifecycle exists' do
      before do
        allow(workflow_client).to receive(:lifecycle).with(druid: pid, milestone_name: 'published').and_return(true)
      end
    ...
    context 'if the published lifecycle does not exist' do
      before do
        allow(workflow_client).to receive(:lifecycle).with(druid: pid, milestone_name: 'published').and_return(false)
      end
