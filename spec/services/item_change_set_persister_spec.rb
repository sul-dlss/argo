      ...
      it 'invokes object client with item/DRO that has new barcode' do
        expect(Repository).to have_received(:store).with(
          cocina_object_with(
            identification: {
              barcode: new_barcode,
              catalogLinks: [{ catalog: CatalogRecordId.type, catalogRecordId: catalog_record_id_before,
                               refresh: true }]
            }
          )
        )
    ...
    context 'when change set has removed catalog_record_id' do
      ...
      it 'invokes object client with item/DRO that has no catalog_record_id' do
        expect(Repository).to have_received(:store).with(
          cocina_object_with(
            identification: {
              catalogLinks: [{ catalog: CatalogRecordId.previous_type, catalogRecordId: catalog_record_id_before,
                               refresh: false }],
              barcode: barcode_before
            }
          )
