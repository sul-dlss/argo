...
feature 'Enable buttons' do
  ...
  scenario 'buttons are enabled if their services return true', js: true do
    ...
    visit catalog_path 'druid:hj185vb7593'
    expect(page).to_not have_css 'a.enabled', text: 'Close Version'
    expect(page).to_not have_css 'a.enabled', text: 'Open for modification'
    expect(page).to_not have_css 'a.enabled', text: 'Republish'
    expect(page).to_not have_css 'a.enabled', text: 'Purge'
  end
end
