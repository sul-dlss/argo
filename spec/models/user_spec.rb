...
describe User, :type => :model do
  ...
  describe '#get_dor_object' do
    ...
    it 'returns a DOR object, as is' do
      ...
      expect(DruidTools::Druid).not_to receive(:new)
      expect(Dor).not_to receive(:find)
      obj = subject.send(:get_dor_object, item)
      expect(obj.pid).to eq(item.pid)
    end
    it 'parses a string input using DruidTools::Druid.new()' do
      expect(DruidTools::Druid).to receive(:new).with(druid).at_least(:once).and_call_original
      subject.send(:get_dor_object, druid)
    end
    it 'converts a valid DRUID into a PID for Dor.find' do
      expect(DruidTools::Druid).to receive(:new).with(druid).at_least(:once).and_call_original
      expect(Dor).to receive(:find).with(pid)
      subject.send(:get_dor_object, druid)
    end
    it 'a valid DRUID that does not exist will raise ActiveFedora::ObjectNotFoundError' do
    ...
    it 'an invalid DRUID string raises ArgumentError' do
      expect(Dor).not_to receive(:find)
      expect{subject.send(:get_dor_object, 'invalid_druid')}.to raise_error(ArgumentError)
    end
  end
