...
RSpec.describe DorObjectWorkflowStatus do
  subject { described_class.new(druid, version: 1) }

  let(:druid) { 'druid:abc123def4567' }
    ...
    context 'when not accessioned' do
      before do
        expect(workflow_client)
          .to receive(:lifecycle).with(druid:, milestone_name: 'accessioned')
                                 .and_return(false)
      end
    ...
    context 'when accessioned and submitted' do
      before do
        expect(workflow_client).to receive(:lifecycle)
          .with(druid:, milestone_name: 'accessioned').and_return(true)
        expect(workflow_client).to receive(:active_lifecycle)
          .with(druid:, milestone_name: 'submitted', version: 1).and_return(true)
      end
      ...
      before do
        expect(workflow_client)
          .to receive(:lifecycle).with(druid:, milestone_name: 'accessioned')
                                 .and_return(true)
        expect(workflow_client)
          .to receive(:active_lifecycle).with(druid:, milestone_name: 'submitted', version: 1)
                                        .and_return(false)
        expect(workflow_client)
          .to receive(:active_lifecycle).with(druid:, milestone_name: 'opened', version: 1)
                                        .and_return(true)
      end
