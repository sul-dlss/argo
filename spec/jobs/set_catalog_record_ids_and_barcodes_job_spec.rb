  ...
  describe "#perform" do
    ...
    context "when catalog_record_id and barcode selected" do
      ...
      it "attempts to update the catalog_record_id/barcode for each druid with correct corresponding catalog_record_id/barcode" do
        ...
        expect(subject).to receive(:update_catalog_record_id_and_barcode).with(change_set4, {catalog_record_ids: [catalog_record_ids[3]], refresh: true}, buffer)
        ...
        expect(change_set1).to have_received(:validate).with(barcode: barcodes[0], catalog_record_ids: ["a12345", "a66233"], refresh: true)
  ...
  describe "#update_catalog_record_id_and_barcode" do
    ...
    context "when a DRO" do
      ...
      context "when modification is not allowed" do
        ...
        it "updates catalog_record_id and barcode and versions objects" do
          expect(subject).to receive(:open_new_version).with(previous_version, "#{CatalogRecordId.label} updated to a12345, a66233. Barcode updated to #{barcode}.").and_return(previous_version)
    ...
    context "when a Collection" do
      context "when modification is allowed" do
        ...
        it "updates catalog_record_id and barcode and does not version objects if not needed" do
          expect(subject).not_to receive(:open_new_version).with(previous_version, "#{CatalogRecordId.label} updated to #{catalog_record_ids[0]}.")
          subject.send(:update_catalog_record_id_and_barcode, change_set, {catalog_record_ids: catalog_record_ids_arg, refresh: true}, buffer)
          expect(object_client).to have_received(:update)
            .with(params: updated_model)
        end
      end
