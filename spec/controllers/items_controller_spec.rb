...
describe ItemsController, :type => :controller do
  ...
  describe 'embargo_update' do
    ...
    it 'should call Dor::Item.update_embargo' do
      ...
      expect(subject).to receive(:save_and_reindex).and_call_original
      expect(subject).to receive(:flush_index).and_call_original
  ...
  describe 'source_id' do
    it 'should update the source id' do
      new_pid = 'druid:ab123cd4567'
      expect(@item).to receive(:set_source_id).with(new_pid)
      expect(@item).to receive(:save)
      expect(Dor::SearchService.solr).to receive(:add)
      post 'source_id', :id => @item.pid, :new_id => new_pid
    end
  end
  describe 'tags' do
    before :each do
  ...
  describe 'tags_bulk' do
    ...
    it 'should remove an old tag and add a new one' do
      tag = 'new:thing'
      expect(@item).to receive(:add_tag).with(tag).and_return(true)
      post 'tags_bulk', :id => @item.pid, :tags => tag
    end
    it 'should remove an old tag and add multiple tags' do
      tags = 'Process : Content Type : Book (flipbook, ltr)  Registered By : labware'
      expect(@item).to receive(:add_tag).twice
      post 'tags_bulk', :id => @item.pid, :tags => tags
    end
  end
  ...
  describe 'resource' do
    it 'should set the object and datastream, then call the view' do
      ...
      # get 'resource', :id => @item.pid, :resource => '0001'
    end
  end
  describe 'add_collection' do
    let(:collection_pid) { 'druid:1234' }
    it 'should add a collection' do
      allow(@item).to receive(:save)
      expect(@item).to receive(:add_collection).with(collection_pid)
      post 'add_collection', :id => @item.pid, :collection => collection_pid
    end
  ...
  describe 'set_collection' do
    ...
    it 'should add a collection if there is none yet' do
      ...
      expect(@item).to receive(:add_collection).with(collection_pid)
      post 'set_collection', :id => @item.pid, :collection => collection_pid, :bulk => true
  ...
  describe 'remove_collection' do
    ...
    it 'should remove a collection' do
      expect(@item).to receive(:remove_collection).with(collection_pid)
      post 'remove_collection', :id => @item.pid, :collection => collection_pid
    end
    it 'should 403 if they are not permitted' do
      expect(subject).not_to receive(:save_and_reindex)
      allow(@current_user).to receive(:is_admin).and_return(false)
  ...
  describe '#workflow_view' do
    ...
    it 'should 404 on missing item' do
      expect(Dor::Item).to receive(:find).with(@item.pid).and_raise(ActiveFedora::ObjectNotFoundError)
      get :workflow_view, id: @item.pid, wf_name: 'accessionWF', repo: 'dor', format: :html
