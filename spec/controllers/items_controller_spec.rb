...
describe ItemsController, :type => :controller do
  ...
  describe 'release_hold' do
    it 'should release an item that is on hold if its apo has been ingested' do
      expect(Dor::WorkflowService).to receive(:get_workflow_status).with('dor', @pid, 'accessionWF', 'sdr-ingest-transfer').and_return('hold')
    ...
    it 'should refuse to release an item that isnt on hold' do
      expect(Dor::WorkflowService).to receive(:get_workflow_status).with('dor', @pid, 'accessionWF', 'sdr-ingest-transfer').and_return('waiting')
  ...
  describe 'source_id' do
    it 'should update the source id' do
      ...
      expect(@item).to receive(:save)
      expect(Dor::SearchService.solr).to receive(:add)
      post 'source_id', :id => @pid, :new_id => new_pid
    end
  end
  describe 'tags' do
    before :each do
  ...
  describe 'resource' do
    it 'should set the object and datastream, then call the view'
