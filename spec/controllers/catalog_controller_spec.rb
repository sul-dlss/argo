  ...
  describe '#show enforces permissions' do
    context 'without a governing APO' do
      ...
      context 'item is an APO' do
        let(:item) do
          # Override the definition of `item` so it's an APO
          # without a governing APO.  It must be `item` for the
          # shared examples used in this context.
        ...
        # recent `let(:item)`, so that will be the apo above.
        it_behaves_like 'APO-independent auth for show'
