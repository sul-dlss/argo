...
class Ability
  ...
  def initialize(user)
    ...
    can :manage_item, Dor::AdminPolicyObject do |dor_item|
      Argo::Ability.can_manage_item?(user.roles(dor_item.pid))
    end

    can :manage_content, Dor::AdminPolicyObject do |dor_item|
      Argo::Ability.can_manage_content? user.roles(dor_item.pid)
    end

    can :manage_desc_metadata, Dor::AdminPolicyObject do |dor_item|
      Argo::Ability.can_manage_desc_metadata? user.roles(dor_item.pid)
    end

    can :manage_item, ActiveFedora::Base do |dor_item|
      if dor_item.admin_policy_object
        Argo::Ability.can_manage_item? user.roles(dor_item.admin_policy_object.pid)
      end
    end

    can :manage_content, ActiveFedora::Base do |dor_item|
      if dor_item.admin_policy_object
        Argo::Ability.can_manage_content? user.roles(dor_item.admin_policy_object.pid)
      end
    end

    can :manage_desc_metadata, ActiveFedora::Base do |dor_item|
      if dor_item.admin_policy_object
        Argo::Ability.can_manage_desc_metadata? user.roles(dor_item.admin_policy_object.pid)
      end
    end
    ...
    can :manage_governing_apo, ActiveFedora::Base do |dor_item, new_apo_id|
      ...
      Argo::Ability.can_manage_item?(user.roles(new_apo_id)) && can?(:manage_item, dor_item)
    end

    can :view_content, Dor::AdminPolicyObject do |dor_item|
      Argo::Ability.can_view_content? user.roles(dor_item.pid)
    end

    can :view_metadata, Dor::AdminPolicyObject do |dor_item|
      Argo::Ability.can_view_metadata? user.roles(dor_item.pid)
    end

    can :view_content, ActiveFedora::Base do |dor_item|
      if dor_item.admin_policy_object
        Argo::Ability.can_view_content? user.roles(dor_item.admin_policy_object.pid)
      end
    end

    can :view_metadata, ActiveFedora::Base do |dor_item|
      if dor_item.admin_policy_object
        Argo::Ability.can_view_metadata? user.roles(dor_item.admin_policy_object.pid)
      end
    end
  end
end
