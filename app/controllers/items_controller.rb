class ItemsController < ApplicationController
  ...
  before_filter :create_obj, :except => [
    :open_bulk, :purge_object, :register,
  ]
  before_filter :allow_modify, :only => [
    :add_collection, :set_collection, :remove_collection,
    :add_file, :delete_file, :replace_file,
    :close_version, :open_version,
    :resource,
    :source_id,
    :set_content_type,
    :tags, :tags_bulk,
    :update_rights,
    :update_attributes, :update_resource, :datastream_update,
    :mods,
  ]
  before_filter :allow_view, :only => [
    :get_file, :get_preserved_file,
  ]
  before_filter :enforce_versioning, :only => [
    :add_collection, :set_collection, :remove_collection,
  ...
  after_filter :save_and_reindex, :only => [
    :apply_apo_defaults,
    :add_collection, :set_collection, :remove_collection,
  ...
  def set_collection
    ...
    respond_to do |format|
      if params[:bulk]
        ...
        if can_set_collection
          msg = 'Collection successfully set'
        else
          msg = 'Collection not set, already has collection(s)'
        end
  ...
  def embargo_update
    # TODO: this could be a more granular permission request, i.e.
    #apo = @object.admin_policy_object
    #@object.can_manage_embargo?(current_user.roles(apo.pid))
  ...
  def allow_modify
    return true if current_user.can_manage?(@object)
    render :status => :forbidden, :text => 'forbidden'
    false
  end

  def allow_view
    return true if current_user.can_view?(@object)
    render :status => :forbidden, :text => 'forbidden'
    false
  end
