...
class CollectionForm
  ...
  def self.model_name
    Struct.new(:param_key, :route_key, :i18n_key, :human).new('collection_form', 'collection', 'collection',
                                                              'Collection')
  end
  ...
  def cocina_model
    ...
    if params[:collection_title].present? || params[:collection_abstract].present?
      reg_params[:description] =
        build_description
    end
  ...
  def build_description
    { title: [{ value: params[:collection_title], status: 'primary' }] }.tap do |description|
      if params[:collection_abstract].present?
        description[:note] =
          [{ value: params[:collection_abstract], type: 'summary' }]
      end
    end
  end
end
